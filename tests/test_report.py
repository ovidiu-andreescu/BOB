...
def test_generate_markdown_report_excludes_rendered_assistant_prompts(
  sample_snapshot,
  sample_fingerprint,
  sample_routes,
  sample_reading_path,
  ...
  """Test that report does not include rendered assistant prompt blocks."""
  report = generate_markdown_report(
    snapshot=sample_snapshot,
    fingerprint=sample_fingerprint,
    routes=sample_routes,
  ...
  assert "Suggested AI" not in report
  assert "assistant prompt" not in report.lower()
  assert "Explain backend/main.py and suggest improvements." not in report
...
def test_get_test_files_excludes_init_py():
  """Test that get_test_files() excludes __init__.py files."""
  from repoquest.sample_loader import load_demo_repo

  snapshot = load_demo_repo()
  test_files = get_test_files(snapshot)

  # No test file should be __init__.py
  init_test_files = [f for f in test_files if f.path.endswith("__init__.py")]
  assert len(init_test_files) == 0, "__init__.py files should not be returned as test files"

  # Should still find actual test files
  actual_test_files = [f for f in test_files if f.name.startswith("test_") and f.name.endswith(".py")]
  assert len(actual_test_files) > 0, "Should find actual test files like test_trips.py"



def test_generate_markdown_report_includes_test_intelligence(
  sample_snapshot, sample_fingerprint
):
  """Test that report includes test intelligence section."""
  test_intelligence = TestIntelligence(
    test_insights=[
      TestInsight(
        test_file="backend/tests/test_trips.py",
        framework="pytest",
        imports=["from backend.main import app"],
        likely_targets=["backend/main.py"],
        covered_routes=["GET /api/trips"],
        missing_cases=[],
        suggested_tests=["Test POST /api/trips with invalid data"],
        quality_signals={"assertion_count": 5, "has_fixtures": False, "has_mocks": False, "has_test_client": True, "skipped_count": 0},
      )
    ],
    missing_coverage=["POST /api/trips", "backend/services/recommendations.py"],
    suggested_tests=["Test DELETE /api/trips/{id} with nonexistent ID"],
    test_plan="In a trusted local clone, run: pytest backend/tests/",
  )
  ...
  assert "## Test Intelligence" in report
  assert "backend/tests/test_trips.py" in report
  assert "Missing Coverage" in report
  assert "Suggested Next Tests" in report
  assert "AI Test Plan" not in report
  assert "assistant prompt" not in report.lower()
