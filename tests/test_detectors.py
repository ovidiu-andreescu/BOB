...
def test_no_false_express_detection_in_fastapi_repo():
  """Test that Express is not falsely detected in FastAPI demo repo."""
  snapshot = load_demo_repo()
  fingerprint = generate_fingerprint(snapshot)

  # Express should not be detected
  framework_names = [f.name for f in fingerprint.frameworks]
  assert "Express" not in framework_names, "Express should not be detected in FastAPI repo"


def test_no_false_flask_detection_in_fastapi_repo():
  """Test that Flask is not falsely detected in FastAPI demo repo."""
  snapshot = load_demo_repo()
  fingerprint = generate_fingerprint(snapshot)

  # Flask should not be detected
  framework_names = [f.name for f in fingerprint.frameworks]
  assert "Flask" not in framework_names, "Flask should not be detected in FastAPI repo"


def test_no_false_nextjs_detection_in_react_vite_repo():
  """Test that Next.js is not falsely detected in React+Vite demo repo."""
  snapshot = load_demo_repo()
  fingerprint = generate_fingerprint(snapshot)

  # Next.js should not be detected
  framework_names = [f.name for f in fingerprint.frameworks]
  assert "Next.js" not in framework_names, "Next.js should not be detected in React+Vite repo"


def test_no_false_python_package_detection_in_demo_repo():
  """Test that Python Package is not falsely detected in demo repo."""
  snapshot = load_demo_repo()
  fingerprint = generate_fingerprint(snapshot)

  # Python Package should not be detected (demo is an app, not a package)
  framework_names = [f.name for f in fingerprint.frameworks]
  assert "Python Package" not in framework_names, "Python Package should not be detected in demo app repo"


def test_demo_repo_detects_only_react_vite_and_fastapi():
  """Test that demo repo detects exactly React+Vite and FastAPI, no false positives."""
  snapshot = load_demo_repo()
  fingerprint = generate_fingerprint(snapshot)
  ...
  false_positives = ["Express", "Flask", "Next.js", "Python Package", "Django"]
  for fp in false_positives:
    assert fp not in framework_names, f"{fp} should not be detected in demo repo"
